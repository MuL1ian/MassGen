...
async def test_mode_bar_stays_within_input_header_at_narrow_width(monkeypatch, tmp_path: Path) -> None:
    ...
    async with app.run_test(headless=True, size=(60, 24)) as pilot:
        ...
        assert "Insert" in _widget_text(input_hint)
...
async def test_mode_bar_uses_single_row_when_space_is_available(monkeypatch, tmp_path: Path) -> None:
    ...
    async with app.run_test(headless=True, size=(140, 28)) as pilot:
        await pilot.pause()
        assert app._mode_bar is not None
        app._mode_bar.set_skills_available(True)
        await pilot.pause()
        ...
        await pilot.pause()
        assert "Planning" in _widget_text(app.query_one("#plan_toggle"))

        app._mode_bar.set_plan_mode("execute")
        await pilot.pause()
        assert "Executing" in _widget_text(app.query_one("#plan_toggle"))

        app._mode_bar.set_plan_mode("analysis")
        await pilot.pause()
        assert "Analyzing" in _widget_text(app.query_one("#plan_toggle"))
...
async def test_welcome_screen_reflows_for_narrow_terminals(monkeypatch, tmp_path: Path) -> None:
    """Welcome screen should use compact content when width is constrained."""
    monkeypatch.setattr(textual_display_module, "get_event_emitter", lambda: None)
    monkeypatch.setattr(
        textual_display_module,
        "get_user_settings",
    ...
    async with app.run_test(headless=True, size=(70, 24)) as pilot:
        ...
        cwd_hint_text = _widget_text(app.query_one("#cwd_hint"))
        welcome_hint = app.query_one("#welcome_hint")
        shortcuts_hint = app.query_one("#welcome_shortcuts_hint")
        cwd_hint = app.query_one("#cwd_hint")
        ...
        assert "Paths are not added yet" in cwd_hint_text
        assert welcome_hint.region.y > shortcuts_hint.region.y
        assert welcome_hint.region.y > cwd_hint.region.y
