...
class SkillsSection(SystemPromptSection):
    ...
    def build_content(self) -> str:
        ...
        content_parts.append('- Preferred (when available): call tool `skills(action="read", skill_name="<skill-name>")`')
        content_parts.append('- Discover available skills with `skills(action="list")` when the tool is present')
        content_parts.append("- Fallback: run command `openskills read <skill-name>` when shell access is available")
